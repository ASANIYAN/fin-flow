...
const SECRET_KEY =
  import.meta.env.VITE_SECRET_KEY || "fin-flow-default-secret-key";
...
export const setToken = (token: string, expiresIn: number) => {
  ...
  Cookies.set("finFlowToken", encryptedToken, {
    expires,
    sameSite: "Strict",
    secure: true,
  });
...
export const getToken = (): string | undefined => {
  const encryptedToken = Cookies.get("finFlowToken");
  if (encryptedToken) {
